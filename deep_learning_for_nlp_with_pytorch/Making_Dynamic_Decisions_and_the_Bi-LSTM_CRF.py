...
class BiLSTM_CRF(nn.Module):
  def __init__(self, vocab_size, tag_to_ix, ix_to_tag, embedding_dim, hidden_dim):
  ...
  def _viterbi_decode(self, lstm_feats):
    ...
    for feat in lstm_feats:
      ...
      # For each tag next_tag, record max(viterbi_var_at_t-1 + transition_score[next_tag])
      for next_tag in range(self.target_size):
        ...
        # for each next_tag, record best score and its corresponding back pointer
  ...
  def forward(self, sentence):  # dont confuse this with _forward_alg above.
    ...
    return score, ' '.join([self.ix_to_tag[t] for t in tag_seq])
